...
1. 주최사가 티켓을 삭제한다.
3. 구매자가 티켓을 검색한다.
4. 구매자가 티켓을 선택하여 사용 예약한다.
5. 구매자가 결제한다. (Sync, 결제서비스)
6. 결제가 완료되면, 결제 & 예약 내용을 구매자에게 알림을 전송한다. (Async, 알림서비스)
7. 예약 내역을 주최사에게 전달한다.
8. 구매자는 본인의 예약 내용 및 상태를 조회한다.
9. 구매자는 본인의 예약을 취소할 수 있다.
12. 예약이 취소되면, 결제를 취소한다. (Async, 결제서비스)
13. 결제가 취소되면, 결제 취소 내용을 구매자에게 알림을 전송한다. (Async, 알림서비스)
14. 티켓이 삭제 된다면, 예약도 취소된다.
...
1. 성능
    1. 구매자와 주최사가 자주 예약관리에서 확인할 수 있는 상태를 마이페이지(프론트엔드)에서 확인할 수 있어야 한다 - CQRS
...
![image](2.png)
    ...
    - 과정중 도출된 잘못된 도메인 이벤트들을 걸러내는 작업을 수행함
        - 티켓검색됨, 예약정보조회됨 :  UI 의 이벤트이지, 업무적인 의미의 이벤트가 아니라서 제외
...
![image](4.png)
    ...
    - 티켓, 예약관리, 결제는 그와 연결된 command 와 event 들에 의하여 트랜잭션이 유지되어야 하는 단위로 그들 끼리 묶어줌
...
![image](5.png)
    ...
    - 도메인 서열 분리 
        - Core Domain:  티켓, 예약 : 없어서는 안될 핵심 서비스이며, 연견 Up-time SLA 수준을 99.999% 목표, 배포주기는 1주일 1회 미만
...
![image](https://github.com/juwarny/example-hotel-books/blob/master/%E1%84%89%E1%85%B3%E1%84%8F%E1%85%B3%E1%84%85%E1%85%B5%E1%86%AB%E1%84%89%E1%85%A3%E1%86%BA%202021-06-04%20%E1%84%8B%E1%85%A9%E1%84%8C%E1%85%A5%E1%86%AB%207.29.59.png)
    ...
    - (ok) 주최사가 티켓을 등록한다.
    - (ok) 주최사가 티켓을 삭제한다.
    - (ok) 주최사가 롬을 삭제 시 예약 취소
    - (ok) 각 이벤트가 알림으로 전달 (Async, 알림 서비스)

    - (ok) 구매자가 티켓을 검색한다.
    - (ok) 구매자가 티켓을 선택하여 사용 예약한다.
    - (ok) 구매자가 결제한다. (Sync, 결제서비스)
    - (ok) 결제가 완료되면, 결제 & 예약 내용을 구매자에게 알림을 전송한다. (Async, 알림서비스)
    - (ok) 예약 내역을 주최사에게 전달한다.
    
    - (ok) 구매자는 본인의 예약 내용 및 상태를 조회한다.
    - (ok) 구매자는 본인의 예약을 취소할 수 있다.
...
![image](https://github.com/juwarny/example-hotel-books/blob/master/%E1%84%89%E1%85%B3%E1%84%8F%E1%85%B3%E1%84%85%E1%85%B5%E1%86%AB%E1%84%89%E1%85%A3%E1%86%BA%202021-06-04%20%E1%84%8B%E1%85%A9%E1%84%8C%E1%85%A5%E1%86%AB%207.29.59.png)
    ...
    - 마이크로 서비스를 넘나드는 시나리오에 대한 트랜잭션 처리
        - 티켓 예약시 결제처리: 결제가 완료되지 않은 예약은 절대 받지 않는다에 따라, ACID 트랜잭션 적용. 예약 완료시 결제처리에 대해서는 Request-Response 방식 처리
...
알림 시스템은 예약/결제와 완전히 분리되어있으며, 이벤트 수신에 따라 처리되기 때문에, 알림 시스템이 장애로 인해 잠시 내려간 상태라도 예약을 받는데 문제가 없다:
```
# 알림 서비스 장애 발생
NAME                              READY   STATUS                       RESTARTS   AGE
...
- Correlation-key로 식별하여 예약(book) 이벤트를 통해 생성된 결제(pay) 건 및 참조된 티켓(ticket) 번호에 대해 티켓 삭제 시 예약이 취소가 되고 동일한 Correlation-key를 가지는 결제(pay) 이벤트 건 역시 삭제되는 모습을 확인한다:
...
결제(pay) 및 예약(book) 이벤트 건을 GET 명령어를 통해 조회한 결과 티켓(ticket)에서 삭제한 3번 키를 갖는 결제 및 예약 이벤트 또한 삭제된 것을 확인
...
```
HTTP/1.1 201     0.06 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     2.16 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.07 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     5.00 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     2.37 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.18 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.18 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.18 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.10 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     2.62 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.08 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     4.89 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     4.68 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     9.93 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.28 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.10 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     2.26 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     0.11 secs:     354 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     2.41 secs:     352 bytes ==> POST http://books:8080/bookss
HTTP/1.1 201     2.34 secs:     352 bytes ==> POST http://books:8080/bookss
...
```
* kubectl describe pod/book-7dc6fbb847-8vwtm -n myticket
```
Containers:
  books:
    Container ID:   docker://541140d9489d0addf1bbe13a0a0c47b978a38cb7660439c548647d1a03f9b2c8
    Image:          879772956301.dkr.ecr.ap-northeast-2.amazonaws.com/user04-books:latest
    Image ID:       docker-pullable://879772956301.dkr.ecr.ap-northeast-2.amazonaws.com/user04-books@sha256:120ff700c1289470af764a6029e852bcc83d3c8a2177900eec8b4a5551ee756d
    Port:           8080/TCP
    company Port:      0/TCP
    State:          Running
    ...
    Requests:
      cpu:      200m
    Liveness:   http-get http://:15020/app-health/books/livez delay=120s timeout=2s period=5s #success=1 #failure=5
    Readiness:  http-get http://:15020/app-health/books/readyz delay=10s timeout=2s period=5s #success=1 #failure=10
    Environment:
...
```
siege -v -c100 -t300S --delay 2S -r10 --content-type "application/json" 'http://books:8080/actuator/health'
```
```
siege -v -c255 -t180S -r10 --content-type "application/json" 'http://books:8080/books POST {
    "startDate": "2012-04-23T18:25:43.511+0000",
    "endDate": "2012-04-27T18:25:43.511+0000",
    "customerId": 1,
    "companyId": 1,
...
```
```
(base) juwonkim@JUWONui-MacBookAir yaml % kubectl get deploy books -n myticket -w
NAME      READY   UP-TO-DATE   AVAILABLE   AGE
books   0/1     1            0           13s
books   1/1     1            1           79s
books   0/1     1            0           2m34s
books   1/1     1            1           4m9s
```
![image](https://github.com/juwarny/example-hotel-books/blob/master/%E1%84%89%E1%85%B3%E1%84%8F%E1%85%B3%E1%84%85%E1%85%B5%E1%86%AB%E1%84%89%E1%85%A3%E1%86%BA%202021-06-04%20%E1%84%8B%E1%85%A9%E1%84%8C%E1%85%A5%E1%86%AB%2011.25.52.png)

![image](https://github.com/juwarny/example-hotel-books/blob/master/%E1%84%89%E1%85%B3%E1%84%8F%E1%85%B3%E1%84%85%E1%85%B5%E1%86%AB%E1%84%89%E1%85%A3%E1%86%BA%202021-06-04%20%E1%84%8B%E1%85%A9%E1%84%8C%E1%85%A5%E1%86%AB%2011.26.59.png)

## CQRS

- 구매자와 주최사가 자주 예약관리에서 확인할 수 있는 상태를 마이페이지(프론트엔드)에서 확인할 수 있어야 한다:
